...
# Input & output files
path_init_files = 'init' # Path to pumping optimization initial conditions
...
qmuni_df = pd.read_csv(os.path.join(path_model_params, qmuni_file), 
                       index_col = 0, delim_whitespace = True, 
                       header = None, names = ['name', 'value'])
qmuni_df.index = [x.split('Q')[-1] for x in qmuni_df.index]
qmuni_df.value = qmuni_df.value / (24*3600)
...
init_df = pd.read_csv(init_filenames, index_col = 0)
hds_file_init = init_df.loc[realnme, 'hds']
zta_file_init = init_df.loc[realnme, 'zta']
...
for well in node_data_df.wellid:
    node_data_df.loc[node_data_df.wellid == well, 'Qm3_s'] = qmuni_df.loc[qmuni_df.index == well, 'value'].values[0]
#=========================================#
