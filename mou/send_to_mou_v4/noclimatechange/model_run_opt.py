...
# working_dir = 'C:/Users/CECOU50/Documents/Cecile Coulon/4_Model Grande Entree/part3_ies/mou/'\
#     'mou_v3_tests/input/50 reals no dv_init realnme_none no hkEDC/'
working_dir = './'
os.chdir(working_dir)

# Path to MODFLOW-2005
# modflow_path = 'mf2005'
modflow_path = '/srv/common/Programmes/bin/mf2005' # Path to Modflow (ENSEGID)

# Path to pumping optimization initial conditions
path_init_files = 'init'
...
realname_file = 'realnme.dat' # name of realization
path_bin_in = 'preproc_IDM_20m_v6.bin' # Bin file generated with QGridder
init_filenames = 'init_filenames.csv' # csv containing initial filenames

name_simu = 'simu_opt' # Model name
...
qmuni_df_m3d = pd.read_csv(os.path.join(path_model_params, qmuni_file), 
                       index_col = 0, delim_whitespace = True, 
                       header = None, names = ['name', 'value'])
qmuni_df_m3d.index = [x.split('Q')[-1] for x in qmuni_df_m3d.index]

qmuni_df_m3s = cp.copy(qmuni_df_m3d)
qmuni_df_m3s.value = qmuni_df_m3s.value / (24*3600)


#%%----------------- Get initial head & interface elevations ------------------

#---- Read current realization name

realnme = str(int(pd.read_csv(os.path.join(path_model_params, realname_file), 
                index_col = 0, delim_whitespace = True, header = None, 
                names = ['name', 'value']).values[0][0]))

if realnme == '9999':
    realnme = 'base'

#---- Find the corresponding initial hds, bynd, zta filenames

init_df = pd.read_csv(init_filenames)
init_df['real_name'] = init_df['real_name'].astype(str)
hds_file_init = init_df.loc[init_df['real_name'] == realnme, 'hds'].values[0]
zta_file_init = init_df.loc[init_df['real_name'] == realnme, 'zta'].values[0]

#---- Read the files (initial conditions: at the last timestep )

# Freshwater heads
hds = flopy.utils.binaryfile.HeadFile(os.path.join(path_init_files, hds_file_init))
times = hds.get_times()
h_init = hds.get_data(totim = times[-1])

# Interface data
zta = flopy.utils.binaryfile.CellBudgetFile(os.path.join(path_init_files, zta_file_init))
...
#============== MODIF: OPT ===============#
for well in node_data_df.wellid:
    node_data_df.loc[node_data_df.wellid == well, 'Qm3_s'] = qmuni_df_m3s.loc[qmuni_df_m3s.index == well, 'value'].values[0]
#=========================================#
...
wel_sp_data_mnw2_df = pd.DataFrame(list(zip(per * active_mnw2_wells, 
                                            node_data_df.wellid, 
                                            - node_data_df.Qm3_s)), #- qmuni_df.value)),#
...
oc_list = ['save head']#['save head', 'save budget']
#spd = {(i,0) : oc_list for i in range(nper)} # Save head & budget for (stress period i, timestep 1)

spd = {(0, nstp - 1): oc_list} # Save head and budget at the end of the simulation only
...
#    spd[ (0, kstp) ] = oc_list # Creat dictionary = {(stress period i, timestep i): save head}
...
# New model
mf = flopy.modflow.Modflow(modelname = name_simu, 
                           model_ws = model_ws, 
                           exe_name = modflow_path)

# Discretization package
discret = flopy.modflow.ModflowDis(mf, nlay = nlay, nrow = nrow, ncol = ncol, 
...
hds = flopy.utils.binaryfile.HeadFile(
        os.path.join(model_ws, name_simu + '.hds'))

times = hds.get_times() #[i/(365.25*24*3600) for i in times]
hswi = hds.get_alldata()[:,:,:,:]


#---- MNW2 data

bynd = pd.read_csv(os.path.join(model_ws, name_simu + '.bynd'), 
...
zta = flopy.utils.binaryfile.CellBudgetFile(
        os.path.join(model_ws, name_simu + '.zta'))
...
hcell_init, hwell_init, zcell_init, zwell_init, zbotm = {}, {}, {}, {}, {}
...
for well in muni_wells_row_col.keys():
    ...
    #---- Define hcell_init & zcell_init values (hwell_init = zcell_init at steady state without pumping)
    
    hcell_init[well] = h_init[0, row, col]
    zcell_init[well] = zeta_init[0, row, col]
    
    hwell_init[well] = hcell_init[well]
