...
def write_df_to_dat(file_name, df_subset):
    
    ''' Export dataframe to .dat file  '''
    
    dest_file = open(file_name + '.dat', 'w')
    
    dest_file.write(df_subset.to_string(
            col_space = 0, formatters = DIC_FMT, 
            justify = "left", header = False, index = False))
    
    dest_file.close()


def write_df_to_tpl(file_name, df_subset):
    
    ''' Write parameters to PEST template (.tpl) file '''
    
    f_tpl = open(file_name + '.tpl', 'w')
    
    f_tpl.write("ptf ~\n")
    
    f_tpl.write(df_subset.to_string(
            col_space = 0, formatters = DIC_FMT, 
            justify = 'left', header = False, index = False))


def write_df_to_ins(file_name, df_subset):
    
    ''' Write dataframe to PEST instruction (.ins) file '''
    
    f_ins = open(file_name + '.ins', 'w')
    
    f_ins.write("pif #\n")
    
    f_ins.write(df_subset.to_string(
            col_space = 0, formatters = DIC_FMT, 
            justify = 'left', header = False, index = False))
